...
describe("combine_inputs()", {
  ...
  it("currently loses column names when an input has exactly one column", {
    # BUG: `df[rows, ]` without drop = FALSE collapses a single-column data
    # frame to a bare vector, so data.frame() names the column after the
    # deparsed subsetting expression instead of keeping the original name.
    species <- data.frame(body_mass = c(10, 20))
    food <- data.frame(digestibility = 0.6)
    environment <- data.frame(air_temperature = c(0, 10, 20))
    ...
    expect_false(all(c("body_mass", "digestibility", "air_temperature") %in% names(out)))
  })
